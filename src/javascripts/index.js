function cookie( key , value , options){
      ...
      // 给options设置默认值; 
      if(!options){ // undefined , null , 0 , false
            options = {};
      }
      ...
      var str = [
            key , "=" , value,
            // options里面的参数有三个， 如果这个参数存在我们就拼接，如果不存在我们就忽略; 
...
class Observer{
      ...
      // 订阅功能; 
      on( callback ,  type = "default" ){
            // 向fans_list之中添加数据分成两种情况; 
            // - 这个类别不存在 : 建立数组之后在进行数据放入; 
            // - 这个类别存在 : 直接进行放入; 
            if(!this.fans_list[type]) this.fans_list[type] = [];
            // * 经过了上面一行代码， 我操作的this.fans_list[type]一定是数组; 
            this.fans_list[type].push( callback );
      }
      // 触发功能; 
      emit( msg = "" , type = "default" ){
